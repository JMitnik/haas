...
aws secretsmanager create-secret --name $JWT_SECRET_NAME --secret-string $JWT_SECRET --region $REGION  --profile haas-prod
aws secretsmanager create-secret --name $API_SECRET_NAME --secret-string $API_SECRET --region $REGION  --profile haas-prod
