...
const seedCompany = async (customer: Customer) => {
  const standardEdges = [
    {
      parentQuestionContains: `How do you feel about ${customer.name}`, // hardcode company name?
    ...
    {
      parentQuestionContains: `How do you feel about ${customer.name}`,
  ...
  const standardSubChildrenWithLeafs = await Promise.all(
    standardSubChildren.map(async rootChild => {
        ...
        where: {
          ...
          AND: {
            id_in: await Promise.all(
              (
                await prisma.questionnaire({ id: questionnaire.id }).leafs()
              ).map(leaf => leaf.id)
            )
          }
        }
  ...
  const standardRootChildrenWithLeafs = await Promise.all(
    standardRootChildren.map(async rootChild => {
      const subleafs = await prisma.leafNodes({
        where: {
          title_contains: rootChild.overrideLeafContains,
          AND: {
  ...
  const rootQuestions = await Promise.all(
    standardRootChildrenWithLeafs.map(async childNode =>
      prisma.createQuestionNode({
        ...
        children: {
          create: childNode.children.map(child => ({
            ...
            options: {
              create: child.childrenNodes.map(subChild => ({
  ...
  const mainQuestions = await prisma.questionNodes({
    where: {
      ...
      AND: {
        id_in: await Promise.all(
          (
            await prisma.questionnaire({ id: questionnaire.id }).questions()
          ).map(q => q.id)
        )
      }
    }
    ...
    standardEdges.map(async edge => {
      ...
      const questionCondition = await prisma.questionConditions({
        where: {
          ...
          id_in: await Promise.all(
            (await prisma.questionConditions()).map(question => question.id)
          ),
  ...
  await Promise.all(
    (await prisma.questionNodes()).map(async node => {
