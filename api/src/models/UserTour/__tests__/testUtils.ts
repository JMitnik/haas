...
export const prepTours = async (prisma: PrismaClient) => {
  const userTourService = new UserTourService(prisma);

  await userTourService.createAndDispatchUserTour({
    type: 'RELEASE',
    triggerVersion: '1.0',
    steps: [],
  });

  const latestRelease = await userTourService.createAndDispatchUserTour({
    type: 'RELEASE',
    triggerVersion: '1.1',
    steps: [],
  });

  const automationGuide = await userTourService.createAndDispatchUserTour({
    type: 'GUIDE',
    triggerPage: 'automation_overview',
    steps: [
      {
        titleKey: 'title_key_1',
        helperKey: 'helper_key_1',
      },
    ],
  });
  ...
  await userTourService.createAndDispatchUserTour({
    type: 'GUIDE',
    triggerVersion: 'user_overview',
    steps: [
      {
        titleKey: 'title_key_2',
        helperKey: 'helper_key_2',
      },
    ],
  });

  return { latestRelease, automationGuide }
}
...
export const clearDatabase = async (prisma: PrismaClient) => {
  if (process.env.NODE_ENV === 'test') {
    await prisma.$transaction([
      ...
      prisma.tourStep.deleteMany({}),
      prisma.tourOfUser.deleteMany({}),
