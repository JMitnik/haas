...
import { GetWorkspaceUsersConnectsQuery, GetWorkspaceUsersConnectsQueryHookResult, GetWorkspaceUsersConnectsQueryVariables } from 'types/generated-types'
...
export const handlers = [
  ...
  graphql.query('GetWorkspaceUsersConnects', (req: GraphQLRequest<GetWorkspaceUsersConnectsQueryVariables>, res, ctx) => {
    const startOffset = req.variables.filter?.offset || 0;
    const nrItems = req.variables.filter?.limit || 0;

    const results: GetWorkspaceUsersConnectsQueryHookResult['data'] = {
      ...GetWorkspaceUsersConnects.data,
      usersConnection: {
        ...GetWorkspaceUsersConnects.data.usersConnection,
        // @ts-ignore
        userCustomers: GetWorkspaceUsersConnects.data.usersConnection.userCustomers.slice(startOffset, startOffset + nrItems),
      }
    };

    return res(ctx.data({...results}));
  }),
