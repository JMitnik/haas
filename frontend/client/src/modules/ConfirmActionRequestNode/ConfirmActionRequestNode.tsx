...
import { LanguageEnumType, useConfirmActionRequestMutation, useVerifyActionRequestMutation } from 'types/generated-types';
...
import { VerifyActionableNodeContainer } from './ConfirmActionRequestNodeStyles';
...
const getHeaderText = (language?: LanguageEnumType) => {
  switch (language) {
    case LanguageEnumType.Dutch:
      return 'Bedankt voor uw antwoord';
    case LanguageEnumType.English:
...
const VerifyActionableNode = () => {
  ...
  const [confirmActionRequest] = useConfirmActionRequestMutation({
    onError: (error) => {
  ...
  useEffect(() => {
    ...
    const bool = intToBool(filter.agree || undefined);
