...
const cache: InMemoryCache = new InMemoryCache({
  ...
  cacheRedirects: {
    Query: {
      dialogue: (_, args) => toIdValue(defaultDataIdFromObject({ __typename: 'Dialogue', id: args?.id }) || ''),
      dialogues: (_, args) => args.ids.map((id: any) => toIdValue(defaultDataIdFromObject({ __typename: 'Dialogue', id: id?.id }) || '')),
      questions: (_, args) => args.ids.map((id: any) => toIdValue(defaultDataIdFromObject({ __typename: 'QuestionNode', id: id?.id }) || '')),
      question: (_, args) => toIdValue(defaultDataIdFromObject({ __typename: 'QuestionNode', id: args?.id }) || ''),
